...
module Solution = struct
  let file ~name ext =
    Filename.concat !Config.solutions (name ^ ext)
  ...
  let write_solver_name ~name solver_name =
    let ochan = open_out (file ~name ".solver") in
    output_string ochan (solver_name ^ "\n");
    close_out ochan

  let add_solver_name ~name solver_name =
  ...
  let write_if_better ~problem ~solver_name solution =
    ...
    if score > score' then
      (
        ...
        write_solver_name ~name solver_name;
        write_solution ~name solution;
      )
    else if score = score' then
      add_solver_name ~name solver_name
end
